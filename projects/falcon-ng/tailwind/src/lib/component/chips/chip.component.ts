...
export class ChipComponent implements OnInit {
  ...
  ngOnInit() {
    setTimeout(() => {
      if (this.control.chipSelectedOptions.length > 0) {
        this.items = this.formGroup.controls[this.control.formControlName] as FormArray;
  ...
  keyboardEnterEvent(event: any): void {
    if (this.control.event !== undefined) this.control.event.keyboardEnter?.emit(event);
